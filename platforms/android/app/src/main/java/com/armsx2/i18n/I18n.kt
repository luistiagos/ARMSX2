...
private val BASE_EN: Map<String, String> = mapOf(
    ...
    "friends.explain" to "Link Discord to show what you're playing and see which friends are in RetroSystem PS2. Uses your existing Discord friends — RetroSystem PS2 keeps no account and runs no server. Only while the app is open.",
    ...
    "friends.playingNow" to "In RetroSystem PS2 now",
    "friends.nobody" to "None of your Discord friends are in RetroSystem PS2 right now.",
    "friends.notify" to "Notify me in game",
    "friends.notify.desc" to "Show a message over the game when a friend starts playing RetroSystem PS2.",
    ...
    "about.pcsx2.description" to "RetroSystem PS2 is built on the open-source PCSX2 emulator.",
    ...
    "app.telemetry" to "Send anonymous telemetry and crash reports",
    "app.telemetry.desc" to "Help diagnose crashes and compatibility problems by sending app, device, error and log details. Game and BIOS files are never sent. Changes apply immediately.",
    ...
    "update.checkOnLaunch.desc" to "Automatically check for a new version each time RetroSystem PS2 starts.",
    ...
    "games.exit.title" to "Exit RetroSystem PS2?",
    ...
    // Catalogo de ROMs (TASK-0023). So em ingles por enquanto: os arquivos de traducao sao
    // gerados a maquina e estas chaves entram na proxima passada.
    "catalog.title" to "ROM catalog",
    "catalog.loading" to "Loading catalog…",
    "catalog.available" to "Available",
    "catalog.search" to "Search",
    "catalog.empty" to "No game matches this search.",
    "catalog.download" to "Tap to download",
    ...
    "keyboard.space" to "Space",
    "keyboard.clear" to "Clear",
    "keyboard.done" to "Done",
    "home.tab.catalog" to "Catalog",
    "home.tab.saved" to "Saved",
    "home.saved.empty.title" to "No saved games yet",
    "home.saved.empty.body" to "Games you download from the catalog or add to your ROM folders will appear here.",
    "home.saved.empty.exploreCatalog" to "Explore Catalog",
    "home.queue.sectionHeader" to "Downloads in Progress",
    "home.saved.sectionHeader" to "Saved Games",
    "home.saved.delete" to "Delete game",
    "home.saved.delete.title" to "Delete game?",
    "home.saved.delete.message" to "This will permanently delete \"%1\$s\" from your device. This action cannot be undone.",
    "home.saved.delete.confirm" to "Delete",
    "home.saved.delete.cancel" to "Cancel",
    "home.saved.delete.success" to "\"%1\$s\" deleted.",
    "home.saved.delete.failed" to "Could not delete the game file.",
    ...
    "catalog.queue.title" to "Downloads",
    "catalog.queue.count" to "%1\$d in progress",
    "catalog.queue.empty.title" to "No downloads",
    "catalog.queue.empty.body" to "Games you download from the library appear here, with progress and controls to pause or cancel.",
    ...
    // A extração do .7z/.zip (TASK-0048). Duas chaves porque o tamanho descomprimido só é
    // conhecido depois de o cabeçalho do arquivo ser aberto: até lá não há porcentagem que valha.
    "catalog.queue.extracting" to "Extracting… (%1\$d%%)",
    "catalog.queue.extracting.starting" to "Extracting…",
    ...
    "catalog.queue.notification.extracting" to "Extracting…",
    ...
    "hotkeys.exitToLauncher.description" to "When a game was launched from another app (e.g. ES-DE), Close Game returns to that app instead of the RetroSystem PS2 library.",
    ...
    "memcard.delete.body" to "Remove \"%s\" from RetroSystem PS2?\n\nThis deletes the card that RetroSystem PS2 keeps in its own memory-card folder. If you imported it from a file on your device, that original file is not touched.",
    ...
    "overlay.osdSize.description" to "Size of the on-screen display drawn over the game — FPS, stats and notifications. Does not affect the app's menus. 100% = PCSX2's default; RetroSystem PS2 ships 65% so it doesn't crowd a handheld screen.",
    ...
    "patches.bundled.header" to "Bundled with RetroSystem PS2",
    "patches.bundled.explain" to "These ship inside RetroSystem PS2 and are applied to this game automatically — you didn't install them. Ones with no name are always on and can't be switched off individually.",
    ...
    "perf.fix.gamedbFixes.desc" to "Master switch for the compatibility fixes below, plus the automatic per-game fixes from RetroSystem PS2's game database. Leave on unless troubleshooting.",
    ...
    "renderer.forceMaliFbFetch.description" to "Testing option — affects MediaTek Mali and Mali-G57 on the Vulkan renderer only, and is ignored on every other GPU and renderer. RetroSystem PS2 normally disables framebuffer fetch on these drivers because they hand back stale or empty destination colour, which shows up as black or missing textures. Mali has no hardware dual-source blend, so with fetch off the emulator has to re-read the destination through a per-primitive barrier — very slow in blend-heavy games. Turn this on to test whether your driver is actually affected: if it is fine you should get a large speedup; if it is not you will see black or missing textures, so turn it back off. OFF by default. Restart the game to apply.",
    "renderer.forceMaliFbFetch.label" to "Force Framebuffer Fetch (MediaTek Mali)",
    "renderer.forcePs2DepthQuantization.description" to "Testing option — restores the PS2's exact 32-bit depth precision on GPUs that switch it off by themselves. On Mali (Vulkan renderer only) and on Apple GPUs, RetroSystem PS2 drops that step because computing it in the shader disables an important tile-GPU optimisation. The cost is that the depth test can wrongly pass, which shows up as flicker, stripes, or geometry appearing through what should be in front of it. Turn this on if a game looks wrong that way, especially if the same game looks right on the OpenGL renderer. Expect to lose some speed. OFF by default. Restart the game to apply.",
    "renderer.forcePs2DepthQuantization.label" to "Force PS2 depth precision",
    ...
    "renderer.importTexturePack.bootFirst" to "Boot a game first so RetroSystem PS2 knows the serial.",
    ...
    "throttle.help.title" to "Your phone is slowing your games down",
    "throttle.help.intro" to "Your phone is a Samsung, and it ships with a service that holds the processor back while you play. That is why games run slow and the sound stutters.\n\nIt is not the emulator, and the emulator cannot switch it off. But you can turn that service off on your phone — once, and with no computer.\n\nWant me to show you how?",
    "throttle.help.stepOf" to "Step %d of %d",
    "throttle.help.step1" to "You are going to install a free app and type one command into it. It takes about 5 minutes.\n\nNothing here deletes your games or your saves, and you can undo it whenever you like.\n\nTake your time: I will show one screen at a time.",
    "throttle.help.step2" to "First, install the app called LADB.\n\nTap the button below — it opens the store straight at the right page. Install it, then come back here.",
    "throttle.help.step3" to "Now we unlock a hidden menu on your phone.\n\nTap the button below to open \"About phone\".\n\nInside it, go to \"Software information\" and tap 7 times in a row on \"Build number\". It may ask for your PIN.\n\nYou will see: \"Developer mode has been enabled\".",
    "throttle.help.step4" to "Tap the button below to open \"Developer options\".\n\nFind \"Wireless debugging\" and switch it on.\n\nYour phone has to be connected to Wi-Fi.",
    "throttle.help.step5" to "Still in \"Wireless debugging\", tap on the NAME (not just the switch) to go inside it.\n\nThen tap \"Pair device with pairing code\".\n\nA 6-digit code and a port number will appear.",
    "throttle.help.step6" to "Careful — this is where almost everybody trips up:\n\nThat code disappears if you leave the screen, and it changes every time.\n\nBefore going on, open LADB in SPLIT SCREEN alongside Settings, so you can see both at once.",
    "throttle.help.step7" to "In LADB, type the port and the 6-digit code shown in Settings.\n\nOnce it connects, LADB shows a command line waiting for you to type.",
    "throttle.help.step8" to "Now the command. Tap the button below to copy it, then paste it into LADB and send it.\n\n%s",
    "throttle.help.step9" to "If the answer looks like:\n\n\"new state: disabled-user\"\n\nit worked. You can close LADB.",
    ...
    "throttle.help.openAbout" to "Open \"About phone\"",
    "throttle.help.openDev" to "Open \"Developer options\"",
    "throttle.help.copyCmd" to "Copy the command",
    "throttle.help.copied" to "Command copied — now paste it into LADB",
    "throttle.help.copyFailed" to "Could not copy. Type the command by hand, exactly as shown.",
    "throttle.help.openFailed" to "This phone does not have that screen. Open it by hand through Settings.",
    "throttle.help.next" to "Next",
    "throttle.help.back" to "Back",
    "throttle.help.close" to "Close",
    "app.throttleWarnings.fix" to "Show the steps",
    "app.throttleWarnings.fixHint" to "Opens the step-by-step guide for switching off the Samsung service that limits game speed.",
    "app.throttleWarnings" to "Samsung speed limit warning",
    "app.throttleWarnings.desc" to "Warns you once, when the app starts, if your phone is a Samsung with Game Optimizing Service enabled — the service that holds the processor back during games. The warning stops on its own once the service is switched off.",
    ...
    "setup.systemDir.intro" to "RetroSystem PS2 stores memory cards, save states, configs, and shader data in app-private storage by default. ",
    ...
    "setup.welcome.heading" to "Welcome to RetroSystem PS2 setup!",
