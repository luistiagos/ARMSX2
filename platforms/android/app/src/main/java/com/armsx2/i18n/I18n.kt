...
private val BASE_EN: Map<String, String> = mapOf(
    ...
    "throttle.dialog.title" to "Your phone is limiting the CPU",
    "throttle.dialog.body.samsung" to "Samsung's Game Optimizing Service is holding this phone's CPU at %d%% of its maximum, and that is why the game is slow. The app cannot switch it off. To get full speed back:\n\n1. Tap \"Open Game Optimizing Service\" below.\n2. On the screen that opens, tap \"Force stop\".\n3. Confirm, then come back to the game.\n\n\"Disable\" is greyed out on purpose — \"Force stop\" is the one that works. The service starts itself again after a while, so repeat this whenever the game slows down again.",
    "throttle.dialog.body" to "This phone is holding the CPU at %d%% of its maximum while the game runs, and that is why it is slow. The limit comes from the phone's own system, not from the emulator, and the app cannot switch it off.",
    "throttle.dialog.open" to "Open Game Optimizing Service",
    "throttle.dialog.close" to "Close",
    "throttle.fix.unavailable" to "Could not open the Game Optimizing Service page on this device.",
    "app.throttleWarnings.fix" to "How to fix it",
    "app.throttleWarnings.fixHint" to "What caps the CPU is Samsung's Game Optimizing Service. Open its page and tap Force stop — full speed comes back immediately. Disable is greyed out there on purpose, and the service starts itself again after a while, so repeat this whenever the game slows down again.",
    "app.throttleWarnings" to "Device throttling warning",
    "app.throttleWarnings.desc" to "Tells you once if the phone holds the CPU below its own maximum while a game runs, which halves emulation speed. On Samsung devices this is the Game Optimizing Service; no app can switch it off, and the only known fix needs a PC: adb shell pm disable-user --user 0 com.samsung.android.game.gos",
