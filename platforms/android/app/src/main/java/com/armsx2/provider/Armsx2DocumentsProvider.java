 ...
 */
/**
 * Expoe a pasta de dados do app no gerenciador de arquivos do sistema.
 *
 * Sem isto, a pasta fica alcancavel so por caminho digitado: `getExternalFilesDir` nao aparece na
 * navegacao normal do Android desde o scoped storage, e e onde vivem as ROMs baixadas, os memory
 * cards e os savestates. Com o provider registrado, "RetroSystem PS2" vira uma origem na tela de
 * "Abrir com" e no app Arquivos, e o usuario copia um save para fora ou uma ROM para dentro sem
 * depender de cabo nem de permissao de todos os arquivos.
 *
 * Portado da arvore anterior. A unica mudanca de substancia e de onde vem a raiz -- ver
 * [#requireBaseDir].
 */
public class Armsx2DocumentsProvider extends DocumentsProvider {
    ...
    public Cursor queryRoots(@Nullable String[] projection) throws FileNotFoundException {
        ...
        row.add(Root.COLUMN_TITLE, context.getString(com.armsx2.R.string.app_name));
        row.add(Root.COLUMN_SUMMARY, context.getString(com.armsx2.R.string.documents_provider_root_summary));
    ...
    private static File requireBaseDir(@NonNull Context context) throws FileNotFoundException {
        // A raiz de dados do fork e `MainActivityRuntime.assetCopyRoot`, nao o
        // `DataDirectoryManager` da arvore anterior: aquele era o modelo antigo do upstream, que
        // esta arvore ja substituiu. E a mesma pasta que o core usa e onde o catalogo grava as
        // ROMs -- que e justamente o que este provider existe para deixar o usuario alcancar pelo
        // gerenciador de arquivos do sistema.
        String root = com.armsx2.runtime.MainActivityRuntime.Companion.assetCopyRoot(
