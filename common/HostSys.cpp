...
// the periodic event stream, every ~33µs on this host. That stream is a kernel
// option (CONFIG_ARM_ARCH_TIMER_EVTSTREAM, advertised as "evtstrm" in AT_HWCAP and
// /proc/cpuinfo) rather than an architectural guarantee — a device without it would
// have no wake-up at all here. Every device this has run on advertises it, so the
// case is noted rather than coded around.
