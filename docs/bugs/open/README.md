...
- [`armsx2-fork/`](armsx2-fork/README.md): **15** bugs (8 na auditoria de 2026-09-03, mais 2 abertos em 2026-09-05, 1 em 2026-09-11 e 4 em 2026-09-21) que afetam, ou cuja causa continua presente,
...
| [Ajuste por jogo não chega à camada lida pelo core](armsx2-fork/ajuste-por-jogo-igual-ao-global-nao-vence-o-gamedb_2026-09-05T20-14.md) | delta do fork | **Metade implementada (2026-09-11), sem prova de aparelho.** Medido em 2026-09-05: um ajuste por jogo feito pela **biblioteca** não gerava `gamesettings/<serial>_<CRC>.ini` nenhum, então o GameDB vencia toda escolha por jogo. A [TASK-0089](../../task/TASK-0089-ajuste-por-jogo-na-biblioteca-cria-a-camada-de-jogo.md) (`23498c4976`) faz o arquivo nascer; os três passos de aparelho dela não rodaram. **Continua aberta** a regra "só o que difere do global", que vale nos dois caminhos. | Sim | **Média** | Generaliza para toda chave disputada pelo banco. Sem aviso ao usuário. Caso concreto da metade aberta: *Auto Flush* off por jogo no God of War não pina (bug abaixo). Bloqueador de diagnóstico: o core grava esses INIs em `0600` e o pacote não é `debuggable`, então nem `adb` nem `run-as` leem o conteúdo. |
| [God of War lento no POCO C75](armsx2-fork/god-of-war-lento-no-poco-c75-mali-g52_2026-09-11T14-36.md) | herdado do upstream (GameDB) sobre hardware no piso | **Sem medição.** Causa provável derivada do código: os 14 seriais de God of War carregam `autoFlush: 1` e o overlay mobile do upstream não relaxa nenhum. O contorno óbvio (*Auto Flush* off por jogo) não pina, pela linha acima; o que funciona é *Correções de Hardware Manuais* por jogo. | Talvez — e só via upstream | **Média** | A [TASK-0095](../../task/TASK-0095-autoflush-do-god-of-war-em-gpu-fraca.md) mede e, se a troca valer, propõe ao overlay do upstream. O `DeviceTier` não reconhece o aparelho como fraco (8 núcleos, 6–8 GB): sem task. |
| [Downloads só em `Android/data`, sem opção de pasta própria](armsx2-fork/catalogo-download-so-em-android-data-sem-opcao-de-pasta-propria_2026-09-21T00-16.md) | delta do fork (regressão de produto) | **Causa confirmada no código (2026-09-21), sem prova de campo.** `romsDir()` é sempre `assetCopyRoot/roms`; o Android apaga essa árvore em desinstalar e em "Limpar dados", e o manifesto não declara `hasFragileUserData`. A 1.0.x tinha "Diretório de Download"; o fork não. Candidato mais provável para o relato de cliente de 2026-09-20 ("os jogos que baixei sumiram", fork, pasta padrão) — nada no código do fork apaga `roms/`. | Sim | **Alta** | Confirmar com o cliente se desinstalou ou limpou dados. Correção em três degraus: `hasFragileUserData`, devolver a pasta de download separada (flavor `github`, POSIX), aviso no primeiro download. |
| [Pasta de download da 1.0.x não é adotada](armsx2-fork/catalogo-pasta-de-download-da-1-0-x-nao-e-adotada-pelo-fork_2026-09-21T00-16.md) | delta do fork (incompatibilidade fork ↔ version1) | **Causa confirmada no código (2026-09-21), sem prova de campo.** A 1.0.x guardava `download_dir_path` além de `data_dir_path`, e a biblioteca dela era essa pasta; `adoptLegacyDataRoot` lê só a segunda (`grep` por `download_dir_path` no fork: zero). Quem escolheu a pasta na 1.0.x instala o fork por cima e perde de vista todos os jogos. O cliente de 2026-09-20 **não** é este caso (nunca mexeu na opção). Inclui a sonda `createNewFile()` de `validateSystemDirWritable`, que trava com arquivo órfão. | Sim | **Alta** | Adotar `download_dir_path` em `romsDirs` na mesma janela da TASK-0030; decidir se vira destino de download. Validar com aparelho de estado legado (o ramo positivo da TASK-0030 continua sem prova). |
| [Pasta de ROMs semeada uma vez não acompanha a raiz de dados](armsx2-fork/biblioteca-pasta-de-roms-semeada-uma-vez-nao-acompanha-a-raiz-de-dados_2026-09-21T00-16.md) | delta do fork | **Causa confirmada no código (2026-09-21), sem prova de campo.** `seedOwnRomsFolder` grava `assetCopyRoot/roms` como caminho absoluto e nunca mais; trocar Armazenamento muda `assetCopyRoot` (download e `markDownloaded`) sem tocar `romsDirs` (varredura). Download feito depois da troca conclui e não aparece em Salvos. | Sim | **Média** | Derivar a pasta própria na hora de varrer (`listOf(romsDir()) + romsDirs`) em vez de semear. Validar com cartão SD: baixar, trocar para SD, baixar de novo. |
| [Varredura que não lê a pasta grava cache vazio e não revarre](armsx2-fork/biblioteca-varredura-que-nao-le-a-pasta-grava-cache-vazio-e-nao-revarre_2026-09-21T00-16.md) | delta do fork | **Causa confirmada no código (2026-09-21), sem prova de campo.** `listFiles()` nulo vira lista vazia sem erro, `saveCache` persiste `[]` por cima do cache bom, `refresh` trata como sucesso, e `load` só revarre quando a lista de pastas muda. Uma abertura com SD desmontado ou permissão caída deixa a biblioteca vazia até o ↻ manual. | Sim | **Média** | Não persistir resultado de pasta não lida; cache vazio com pastas configuradas dispara revarredura. Reproduzível revogando "todos os arquivos" ou desmontando o SD. |
